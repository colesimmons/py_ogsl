...
class Sign(BaseModel):
    ...
    @classmethod
    def from_lines(cls, lines: List[str]) -> Optional["Sign"]:
        if not lines:
            return None
        ...
        sign = Sign(
            name=first_line_val, is_deprecated=line_type == _LineType.NameDeprecated
        )
        ...
        for i, line in enumerate(lines[1:-1]):
            ...
            if line_type in [
                ...
                value_blocks, form_blocks = _separate_out_value_and_form_blocks(
                    lines[i + 1 : -1]
                )
    ...
    def _assign_property(self, line_type: _LineType, val: str):
        if line_type == _LineType.Aka:
            ...
            raise ValueError(f"Unrecognized line type '{line_type}' in block {lines}.")
...
def _separate_out_value_and_form_blocks(
    lines: List[str],
